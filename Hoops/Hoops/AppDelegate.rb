...
class AppDelegate
  ...
  #
  # Returns the directory the application uses to store the Core Data store file. This code uses a directory named "Hoops" in the user's Library directory.
  #
  def applicationFilesDirectory
    ...
    library_url.URLByAppendingPathComponent("Hoops")
  end
  ...
  def managedObjectModel
    unless @managedObjectModel
      model_url = NSBundle.mainBundle.URLForResource("Hoops", withExtension:"momd")
  ...
  def persistentStoreCoordinator
    ...
    url = directory.URLByAppendingPathComponent("Hoops.sqlite")
  ...
  def seedData
    player1 = NSEntityDescription.insertNewObjectForEntityForName("Player", inManagedObjectContext:self.managedObjectContext)
    player1.firstName = "Kendrick"
    player1.lastName = "Perkins"
    
    player2 = NSEntityDescription.insertNewObjectForEntityForName("Player", inManagedObjectContext:self.managedObjectContext)
    player2.firstName = "Kevin"
    player2.lastName = "Durant"
  ...
  def printData
    ...
    fetch.setEntity(NSEntityDescription.entityForName("Player", inManagedObjectContext:self.managedObjectContext))
    fetch.setPredicate(NSPredicate.predicateWithFormat("lastName == %@", "Perkins"))
    players = self.managedObjectContext.executeFetchRequest(fetch, error:nil)
